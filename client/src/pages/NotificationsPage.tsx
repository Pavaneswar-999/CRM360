...
export function NotificationsPage() {
  ...
  return <div className="page-stack">
    <div className="page-header"><div><span className="eyebrow">Work updates</span><h1>Notifications</h1><p className="page-subtitle">Assignments, stage changes, and tasks with an upcoming due date.</p></div><button className="button button-quiet" onClick={() => void markAll()}><CheckCheck size={16} /> Mark all read</button></div>
    <div className="card notification-card">
      {items.length ? items.map((item) => <article className={`notification-row ${item.read ? 'read' : ''}`} key={item._id}>
        <span className={`notification-icon ${item.type.includes('task') ? 'amber-bg' : item.type.includes('stage') ? 'signal-bg' : 'neutral-bg'}`}>{item.type.includes('task') ? <Clock3 size={17} /> : item.type.includes('stage') ? <Target size={17} /> : <Bell size={17} />}</span>
