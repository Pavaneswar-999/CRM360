...
router.get('/', asyncHandler(async (req, res) => { const items = await Notification.find({ recipient: req.user!.id }).sort({ createdAt: -1 }).limit(40).lean(); res.json({ success: true, data: { items, unreadCount: items.filter((item) => !item.read).length } }) }))
